...
F7 never transmits prompt text, file contents, email or chat messages, screenshots, clipboard contents, or browsing history. Mode 3 — Interpret (opt-in) processes screen frames locally on-device; frames never leave the device, and current macOS builds may use a temporary OS-local PNG that is read and deleted after inference. See [What We Never Collect](/privacy/data-collection#what-f7-never-captures) for the full list.
